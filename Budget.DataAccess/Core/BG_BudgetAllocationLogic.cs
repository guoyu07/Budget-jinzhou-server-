...
namespace BudgetWeb.BLL
{
    public class BG_BudgetAllocationLogic
    {
        ...
        public static DataTable GetAAMon(int depid, int piid, int year)
        {
            ...
            try
            {
                string str = string.Format(" select sum(BAAMon) as BAAMon  ,sum(SuppMon) as SuppMon  from dbo.BG_BudgetAllocation where BAAYear={0} and DepID={1} and PIID={2}", year, depid, piid);
