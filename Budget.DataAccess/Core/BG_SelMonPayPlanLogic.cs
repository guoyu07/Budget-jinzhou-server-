...
namespace BudgetWeb.BLL
{
    public class BG_SelMonPayPlanLogic
    {
        ...
        {
            string sqlStr = "select * from vMonPlayRemark left join BG_PayIncome on vMonPlayRemark.PIID=BG_PayIncome.PIID left join  dbo.BG_Department on vMonPlayRemark.deptid=BG_Department.DepID where deptid={0} and  year(MPTime)={1} and (({2}=0) or ({2}<>0 and month(MPTime)={2})) and (({3}=0)or ({3}<>0 and MPFundingAddTimes={3}))";
