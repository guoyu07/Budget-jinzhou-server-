...
namespace BudgetWeb.BLL
{
 ...
 + "where  a.FaDepID in  (select  FaDepID from   [dbo].[BG_Department] where DepID ={1})", year, deptid);
        ...
        public static DataTable GetDepByfadepid(int AreaDepID, int year)
        {
            string sql = string.Format("select * ,isnull(EditCount1,0)as EditCount from (select *  from BG_Department left join (select count(*) as EditCount1,DepID as DeptID from  BG_Amount  where BGAMYear={1} group by DepID) as a on a.DeptID=BG_Department.DepID where FaDepID= {0} )as a", AreaDepID, year);
            DataTable dt = null;
            try
            {
